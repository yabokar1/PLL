...
def llama_entity_dict(user_text):
 user_text_list = user_text.split(',')
 entity_dict = {}
 print(user_text_list)
...
def deanonymize_data_presidio(user_text, anonymized_text):
...
def deanonymize_data_llama(user_text, anonymized_text):
    ...
    for index, item in enumerate(anonymized_list):
        ...
        if item == '<NAME>':
            index = anonymized_list.index(item)
            anonymized_list[index] = entity_dict['<NAME>'] 
        if item == '<NAME>,\n\nWe':
            index = anonymized_list.index(item)
            anonymized_list[index] = entity_dict['<NAME>'] + ',We'
        if item == '<NAME>,\n\nI':
            index = anonymized_list.index(item)
            anonymized_list[index] = entity_dict['<NAME>'] + ',I'
        if item == '<STUDENT_ID>,' or item == '<STUDENT_ID>':
            index = anonymized_list.index(item)
            anonymized_list[index] = entity_dict['<STUDENT_ID>'] 
        if item == '<PASSPORT_ID>\n\n' or item == '<PASSPORT_ID>.\n\n' or item == '<PASSPORT_ID>' or item == '<PASSPORT_ID>.':
...
def anonymize_data(user_data, user_input):
    try:
        if user_data == "":
            print(AppConfig.ANALYZE_URL)
            # anonymize_text = presidio(user_input)
            anonymize_text = llama(user_input)
            return anonymize_text 
        else:
            response = openai_api(user_data, user_input)
            deanonmyized_response = deanonymize_data_llama(user_data, response)
...
def openai_api(anonymize_text,user_prompt):
